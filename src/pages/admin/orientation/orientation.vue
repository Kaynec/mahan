...
export default defineComponent({
  ...
  setup() {
    ...
    const options = reactive({
      gridName: 'orientation-grid',
      url: `${baseUrl}orientation/findByFieldAndFilter`,
      type: 'GET',
      data: (d: any) => {
        d.filter = {};
    ...
    const editOrientation = (orientation: any) => {
      router.push({
    ...
    const deleteOrientation = (orientation: any) => {
      const allQuestions = async () => {
        const Questions = await SessionServiceApi.getAll({
          orientation: { _id: orientation._id }
        })
          .then((res) => {
      ...
      allQuestions().then((res) => {
        if (res === true) {
          ...
          alertify.confirm('حذف', 'آیا اطمینان دارید؟', function (e: any) {
            if (e) {
              OrientationServiceApi.delete(orientation._id).then((result) => {
      ...
      if (grid.value.getDatatable()) {
        grid.value
          ...
          .on('click', '[data-edit-id]', (e: any) => {
            ...
            if (filteredData.length > 0) editOrientation(filteredData[0]);
          });
        grid.value
          .getDatatableBody()
          .on('click', '[data-delete-id]', (e: any) => {
            ...
            if (filteredData.length > 0) deleteOrientation(filteredData[0]);
          });
      }
    });
